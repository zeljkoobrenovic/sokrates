...
public class ScalaHeuristicDependenciesExtractor extends HeuristicDependenciesExtractor {
    ...
    public List<DependencyAnchor> extractDependencyAnchors(SourceFile sourceFile) {
        ...
        int startIndexOfNamespaceName = content.indexOf("package ");
        if (startIndexOfNamespaceName >= 0) {
            int endIndexOfNamespaceName = content.indexOf("\n", startIndexOfNamespaceName + "package ".length());
            if (endIndexOfNamespaceName >= 0) {
                String namespaceName = content.substring(startIndexOfNamespaceName + "package ".length(), endIndexOfNamespaceName).trim();
                ...
                dependencyAnchor.getDependencyPatterns().add("import .*" + namespaceName.replace(".", "[.]") + "([.][A-Z].*|[.][*]|[.]_|[{].*|)");
