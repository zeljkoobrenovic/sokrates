...
public class LandscapeAnalyzer {
    ...
    // Reads a single entry (by its path relative to data/, e.g. "analysisResults.json" or
    // "text/aspect_main.txt") from a repository's data/data.zip. Repositories are now packaged as a
    // single data.zip (see DataExporter.zipDataFolder); returns null when the zip or entry is
    // absent (e.g. a repository not regenerated with the current Sokrates) so callers can skip it.
    private static String readDataZipEntry(File dataFolder, String entryName) {
        ...
        try (ZipFile zip = new ZipFile(zipFile)) {
            ...
            LOG.warn("Could not read " + entryName + " from " + zipFile.getPath(), e);
            return null;
        }
    }
    ...
    private CodeAnalysisResults getRepositoryAnalysisResults(File repositoryAnalysisResultsFile) {
        try {
            // analysisResults.json + config.json now live inside the repository's data/data.zip
            // (repositoryAnalysisResultsFile names the analysisResults.json path; its parent is the
            // data/ folder). Skip the repository if it has no data.zip (not regenerated yet).
            File dataFolder = repositoryAnalysisResultsFile.getParentFile();
            String json = readDataZipEntry(dataFolder, "analysisResults.json");
            if (json == null) {
                LOG.warn("Skipping repository: no analysisResults.json in " + new File(dataFolder, DATA_ZIP_FILE_NAME).getPath()
                        + " (regenerate this repository's Sokrates reports).");
                return null;
            }
            ...
            try {
                ...
                String configJson = readDataZipEntry(dataFolder, "config.json");
                if (configJson != null) {
    ...
    private void enrichFilesWithCommitHistory(List<FileExport> files, SokratesRepositoryLink sokratesRepositoryLink) {
        ...
        String historyContent = readDataZipEntry(dataFolder, "text/mainFilesWithHistory.txt");
        if (historyContent == null) {
            return;
        }
        try {
            List<String> lines = Arrays.asList(historyContent.split("\n", -1));
            if (lines.isEmpty()) {
                return;
            }
    ...
    private List<FileExport> getRepositoryFilesByScope(String repositoryName, SokratesRepositoryLink sokratesRepositoryLink, String scopeFile) {
        try {
            File dataFolder = getRepositoryAnalysisFile(sokratesRepositoryLink).getParentFile();
            String content = readDataZipEntry(dataFolder, "text/" + scopeFile);
            if (content != null) {
