...
public class LandscapeAnalyzer {
    ...
    // Reads a single data entry (by its path relative to data/, e.g. "analysisResults.json" or
    // "text/aspect_main.txt") from a repository's data folder. Repositories generated by the current
    // Sokrates package data/ into a single data.zip (see DataExporter.zipDataFolder), so the zip is
    // tried first; for repositories generated by an older version (no data.zip) it falls back to the
    // loose file at data/<entryName>. Returns null when neither is present so callers can skip.
    private static String readDataEntry(File dataFolder, String entryName) {
        String fromZip = readDataZipEntry(dataFolder, entryName);
        if (fromZip != null) {
            return fromZip;
        }
        // Fallback: loose file (older reports, before data.zip packaging). Entry names are paths
        // relative to data/, which map directly to loose files (e.g. data/text/aspect_main.txt).
    ...
    // Reads a single entry from a repository's data/data.zip, or null if the zip or entry is absent.
    ...
    private CodeAnalysisResults getRepositoryAnalysisResults(File repositoryAnalysisResultsFile) {
        try {
            ...
            String json = readDataEntry(dataFolder, "analysisResults.json");
            if (json == null) {
            ...
            try {
                ...
                String configJson = readDataEntry(dataFolder, "config.json");
                if (configJson != null) {
