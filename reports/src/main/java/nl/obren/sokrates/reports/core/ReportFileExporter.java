...
public class ReportFileExporter {
    ...
    private static void addVisuals(RichTextReport report, CodeAnalysisResults analysisResults) {
        ...
        analysisResults.getLogicalDecompositionsAnalysisResults().forEach(logicalDecomposition -> {
            ...
            report.addHtmlContent("<td></td>");
            report.addHtmlContent("<td colspan='2'>Components</td>");
            report.addHtmlContent("<td colspan='1'>Dependencies</td>");
            report.addHtmlContent("<td colspan='2'>Commit Racing Charts</td>");
            report.addHtmlContent("</tr>");
            report.startTableRow();
            report.addTableCell(logicalDecomposition.getKey());
            report.startTableCell();
            report.addNewTabLink("Bubble<br>Chart", "visuals/bubble_chart_components_" + index[0] + ".html");
            report.endTableCell();
            report.startTableCell();
            report.addNewTabLink("Tree<br>Map", "visuals/tree_map_components_" + index[0] + ".html");
            report.endTableCell();
            report.startTableCell();
            report.addNewTabLink("Duplication", "visuals/duplication_dependencies_" + index[0] + ".svg");
            report.endTableCell();
            report.startTableCell();
            report.addNewTabLink("All Time Cumulative", "visuals/racing_charts_component_commits_" + index[0] + ".html");
            report.endTableCell();
            report.startTableCell();
            report.addNewTabLink("12 Months WIndow", "visuals/racing_charts_component_commits_12_months_window_" + index[0] + ".html");
            report.endTableCell();
            report.endTableRow();
        });
        report.endTable();
