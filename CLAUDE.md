...
2. **Client-rendered templates** — `common/src/main/resources/templates/*.html`. These are static HTML shells with `${...}` placeholders; `common/.../renderingutils/ExplorerTemplate.render(...)` substitutes `${data}` with the compact JSON serialization of a value object (Jackson, via `JsonGenerator`) and any extra placeholders (e.g. `${langIcons}`, `${features}`, `${options}`). All tables/charts are then built in the browser from the embedded JSON. Used by the landscape `repositories.html` (template `repositories-report.html`) and `files-explorer.html`. This keeps very large landscapes small (an 800+ repo `repositories.html` is ~3 MB of JSON instead of tens of MB of pre-rendered SVG) and adds client-side search + click-to-sort. The per-repository JSON shape is `reports/.../landscape/data/RepositoryExport.java` (+ `RepositoryReportData.java`); `LandscapeDataExport.java` builds the data and renders the templates. When changing the report, the template (JS render logic) and `RepositoryExport` (JSON fields) move together. Validate template JS by extracting the `<script>` blocks, stubbing the placeholders, and running `vm.compileFunction` under node.
